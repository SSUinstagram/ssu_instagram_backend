...
Follows.get('/user',async(req,res)=>{
    ...
    Follows.get('/table',async(req,res)=>{
        try{
            ...
            console.log(user_id)
            const result = await Follow.findAll({where:{following_id:user_id},attributes:{exclude:['number']}});
        ...
        Follows.post('/update',async(req,res)=>{
            try{
                ...
                if(!state)
                {
                    const result = await Follow.create({following_id:following_id,followed_id:followed_id})
                    console.log("팔로우");
                    return res.status(200).json(result);
                }
                else{
                    const result = await Follow.destroy({where:{following_id:following_id,followed_id:followed_id}});
                    console.log("언팔로우");
                    console.log(result);
                    return res.status(200).json(result);
                }
