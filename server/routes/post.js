...
Write.post("/uploadImages", upload.array("img", 1000), async (req, res) => {
  try {
    ...
    if (req.files.length > 5 || req.files.length === 0)
      ...
      else return res.status(400).send("다시 이미지를 선택하여 넣어주세요");
...
Write.post("/uploadPost", async (req, res) => {
  try {
    ...
    await Post.create({ id: id, date: date, contents: contents })
      .then((result) => {
...
Write.post("/updateImages", upload.array("img", 1000), async (req, res) => {
  try {
    ...
    images = images.filter((element, i) => element !== "undefined");
...
Write.get("/filterPage", async (req, res) => {
  ...
  try {
    if (type === "id") {
      ...
      let page = 0;
      // console.log(temp);
      if (temp === false) page = parseInt(result / 9) + 1;
      else {
...
Write.get("/getEachImages", async (req, res) => {
  ...
  try {
    const result = await Image.findOne({ where: { number: number } })
      .then((result) => {
        return res.status(200).send(result);
      })
      .catch((err) => {
